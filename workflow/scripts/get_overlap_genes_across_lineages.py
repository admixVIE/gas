...
for g in all_genes:
    row = {"gene": g}
    for l in lineages:
        row[l] = "Y" if g in genes[l] else "N"
    rows.append(row)

out_df = pd.DataFrame(rows, columns=["gene"] + lineages)
out_df["count"] = (out_df[lineages] == "Y").sum(axis=1)
out_df = out_df.sort_values(["count", "gene"], ascending=[False, True])
out_df = out_df[out_df["count"] > 1]
out_df.to_csv(snakemake.output.genes, sep="\t", index=False)
